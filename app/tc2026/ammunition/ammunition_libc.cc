...
void *ammunition_memcpy(void *dest, const void *src, size_t size) {
    size_t i;
    for (i = 0; i < size; i++) ((unsigned char *)dest)[i] = ((unsigned char *)src)[i];
...
void *ammunition_memset(void *s, int c, size_t n) {
    size_t i;
    for (i = 0; i < n; i++) ((unsigned char *)s)[i] = (unsigned char)c;

    return s;
}

int ammunition_memcmp(const void *mem1, const void *mem2, size_t size) {
    const unsigned char *p1 = (const unsigned char *)mem1, *p2 = (const unsigned char *)mem2;
    while (size--)
        if (*p1 != *p2)
            return (*p1 - *p2);
        else
...
int ammunition_sprintf_d(char *s, int number) {
    ...
    copyOfNumber = number;
    do {
        s[--writePos] = 48 + ((copyOfNumber >= 0 ? copyOfNumber : -copyOfNumber) % 10);
        copyOfNumber /= 10;
    } while (copyOfNumber != 0);
