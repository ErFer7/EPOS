...
int ammunition_add_unsigned_integer_without_overflow_reaction(int size,
                                                              const void *op1,
                                                              const void *op2,
                                                              void *result) {
    int digit_num;
    ...
    for (digit_num = size - 1, carry = 0; digit_num >= 0; digit_num--) {
        sum = (((unsigned char *)op1)[digit_num] + ((unsigned char *)op2)[digit_num] + carry);
        if (sum > UCHAR_MAX) {
            sum -= UCHAR_MAX + 1;
            carry = 1;
        } else
...
int ammunition_subtract_unsigned_integer_without_overflow_reaction(int size,
                                                                   const void *op1,
                                                                   const void *op2,
                                                                   void *result) {
    int digit_num;
    ...
    for (digit_num = size - 1, carry = 0; digit_num >= 0; digit_num--) {
        subtraction = (((unsigned char *)op1)[digit_num] - ((unsigned char *)op2)[digit_num] - carry);
        if (subtraction < 0) {
...
void ammunition_make_complementary_code(int size, const void *operand, void *result) {
    int digit_num;
    int carry;
    int subtraction;
...
int ammunition_multiply_unsigned_integer_by_digit_without_overflow_reaction(int size,
                                                                            void *operand,
                                                                            unsigned int digit) {
    int digit_num;
    unsigned int carry;
...
void ammunition_arithmetic_overflow_reaction(void) {}

/* Originally reaction on all integer and unsigned integer overflow is
   equal to the following function.  The function does nothing. */

void ammunition_arithmetic_unsigned_overflow_reaction(void) {}
...
void ammunition_add_unsigned_integer(int size, const void *op1, const void *op2, void *result) {
    ammunition_overflow_bit = ammunition_add_unsigned_integer_without_overflow_reaction(size, op1, op2, result);
    if (ammunition_overflow_bit != 0) ammunition_arithmetic_unsigned_overflow_reaction();
}
...
void ammunition_add_integer(int size, const void *op1, const void *op2, void *result) {
    int op1_sign;
    int sign_equality;
    ...
    if (ammunition_overflow_bit != 0) ammunition_arithmetic_overflow_reaction();
}
...
void ammunition_subtract_unsigned_integer(int size, const void *op1, const void *op2, void *result) {
    ammunition_overflow_bit = ammunition_subtract_unsigned_integer_without_overflow_reaction(size, op1, op2, result);
    if (ammunition_overflow_bit != 0) ammunition_arithmetic_unsigned_overflow_reaction();
}
...
void ammunition_subtract_integer(int size, const void *op1, const void *op2, void *result) {
    int op1_sign;
    int sign_unequality;
...
int ammunition_multiply_unsigned_integer_without_overflow_reaction(int size,
                                                                   const void *op1,
                                                                   const void *op2,
                                                                   void *result) {
    int op1_digit_num;
    ...
    for (op2_digit_num = size - 1; op2_digit_num >= 0; op2_digit_num--) {
        if (((unsigned char *)op2)[op2_digit_num] != 0) {
            for (op1_digit_num = size - 1, carry = 0; op1_digit_num >= 0; op1_digit_num--) {
                partial_sum = (((unsigned char *)op1)[op1_digit_num] * ((unsigned char *)op2)[op2_digit_num] +
                               long_result[op1_digit_num + op2_digit_num + 1] + carry);
                long_result[op1_digit_num + op2_digit_num + 1] = (unsigned char)(partial_sum % (UCHAR_MAX + 1));
    ...
    overflow_flag = 0;
    for (result_digit_number = size - 1; result_digit_number >= 0; result_digit_number--) {
...
void ammunition_multiply_unsigned_integer(int size, const void *op1, const void *op2, void *result) {
    ammunition_overflow_bit = ammunition_multiply_unsigned_integer_without_overflow_reaction(size, op1, op2, result);
...
void ammunition_multiply_integer(int size, const void *op1, const void *op2, void *result) {
    ...
    ammunition_overflow_bit =
        ammunition_multiply_unsigned_integer_without_overflow_reaction(size, abs_op1, abs_op2, result);
    unsigned_result_sign = INTEGER_SIGN(result);
    if (negative_result_flag) ammunition_make_complementary_code(size, result, result);
    if (unsigned_result_sign && (!negative_result_flag || INTEGER_SIGN(result) != unsigned_result_sign))
        /* Unsigned result can not be represented as integer. */
        ammunition_overflow_bit = 1;
    if (ammunition_overflow_bit) ammunition_arithmetic_overflow_reaction();
}
...
int ammunition_divide_unsigned_integer_without_overflow_reaction(int size,
                                                                 const void *op1,
                                                                 const void *op2,
                                                                 void *result) {
    ...
    for (op2_digit_number = 0; op2_digit_number < size; op2_digit_number++) {
        if (((unsigned char *)op2)[op2_digit_number] != 0) break;
    }
    ...
    if (first_nonzero_digit_number == size) {
        ...
        for (digit_num = 0; digit_num < size; digit_num++) {
            divisable = (remainder * (UCHAR_MAX + 1) + ((unsigned char *)result)[digit_num]);
    ...
    *scaled_op1 = 0;
    ...
    ammunition_multiply_unsigned_integer_by_digit_without_overflow_reaction(size + 1, scaled_op1, scale);

    ammunition_memcpy(normalized_op2, op2, (size_t)size);

    ammunition_multiply_unsigned_integer_by_digit_without_overflow_reaction(size, normalized_op2, scale);

    for (scaled_op1_digit_num = 0; scaled_op1_digit_num <= first_nonzero_digit_number; scaled_op1_digit_num++) {
        ...
        */
        if (scaled_op1[scaled_op1_digit_num] == normalized_op2[first_nonzero_digit_number])
            q_approximation = UCHAR_MAX;
        else
            q_approximation =
                (scaled_op1[scaled_op1_digit_num] * (UCHAR_MAX + 1) + scaled_op1[scaled_op1_digit_num + 1]) /
                normalized_op2[first_nonzero_digit_number];

        while (normalized_op2[first_nonzero_digit_number + 1] * q_approximation >
               (((unsigned long long int)scaled_op1[scaled_op1_digit_num] * (UCHAR_MAX + 1) +
                 scaled_op1[scaled_op1_digit_num + 1] - q_approximation * normalized_op2[first_nonzero_digit_number]) *
                    (UCHAR_MAX + 1) +
        ...
        *extended_normalized_op2 = 0;
        ammunition_multiply_unsigned_integer_by_digit_without_overflow_reaction(
            size - first_nonzero_digit_number + 1, extended_normalized_op2, q_approximation);
        if (ammunition_subtract_unsigned_integer_without_overflow_reaction(size - first_nonzero_digit_number + 1,
            ...
            *extended_normalized_op2 = 0;
            ...
            ammunition_add_unsigned_integer_without_overflow_reaction(size - first_nonzero_digit_number + 1,
                                                                      scaled_op1 + scaled_op1_digit_num,
                                                                      extended_normalized_op2,
                                                                      scaled_op1 + scaled_op1_digit_num);
        }
        ((unsigned char *)result)[size - 1 - first_nonzero_digit_number + scaled_op1_digit_num] = q_approximation;
    }
...
void ammunition_divide_unsigned_integer(int size, const void *op1, const void *op2, void *result) {
    ammunition_overflow_bit = ammunition_divide_unsigned_integer_without_overflow_reaction(size, op1, op2, result);
    if (ammunition_overflow_bit) ammunition_arithmetic_unsigned_overflow_reaction();
}
...
void ammunition_divide_integer(int size, const void *op1, const void *op2, void *result) {
    int negative_result_flag;
    unsigned char op1_complementary[MAX_INTEGER_OPERAND_SIZE];
    unsigned char op2_complementary[MAX_INTEGER_OPERAND_SIZE];
    unsigned const char *abs_op1;
    ...
    if (INTEGER_SIGN(op2)) {
        ...
        abs_op2 = (unsigned const char *)op2;
    ammunition_overflow_bit =
        ammunition_divide_unsigned_integer_without_overflow_reaction(size, abs_op1, abs_op2, result);
    unsigned_result_sign = INTEGER_SIGN(result);
    if (negative_result_flag) ammunition_make_complementary_code(size, result, result);
    if (unsigned_result_sign && (!negative_result_flag || INTEGER_SIGN(result) != unsigned_result_sign))
        /* Unsigned result can not be represented as integer. */
...
void ammunition_unsigned_integer_remainder(int size, const void *op1, const void *op2, void *result) {
    unsigned char temporary[MAX_INTEGER_OPERAND_SIZE];

    ammunition_divide_unsigned_integer(size, op1, op2, temporary);
    if (ammunition_overflow_bit) /* Reaction on zero is called from `divide_unsigned_integer'. */
        ammunition_memset(result, 0, (size_t)size);
    else {
...
void ammunition_unsigned_integer_shift_right(int size, const void *operand, int bits, void *result) {
    int byte_number;
    unsigned byte;
    unsigned carry;
    int bit_shift;
    ...
    if (bits < 0)
        ...
        for (byte_number = (byte_shift >= size ? 0 : size - byte_shift); byte_number < size; byte_number++)
        ...
        if (byte_shift >= size)
            ...
            ammunition_memmove((char *)result + byte_shift, operand, (size_t)(size - byte_shift));
            ammunition_memset(result, 0, (size_t)byte_shift);
            if (bit_shift == 0) return;
            for (byte_number = byte_shift, carry = 0; byte_number < size; byte_number++) {
                byte = ((unsigned char *)result)[byte_number];
                ((unsigned char *)result)[byte_number] = carry | (byte >> bit_shift);
                carry = (byte << (CHAR_BIT - bit_shift)) & UCHAR_MAX;
            }
            if (carry != 0) ammunition_overflow_bit = 1;
        }
        if (ammunition_overflow_bit) ammunition_arithmetic_unsigned_overflow_reaction();
    }
}
...
void ammunition_integer_shift_right(int size, const void *operand, int bits, void *result) {
    int byte_number;
    unsigned byte;
    unsigned carry;
    int bit_shift;
    ...
    if (bits < 0)
        ...
        if (byte_shift >= size)
            ...
            if (bit_shift == 0) return;
            carry = (((operand_sign ? UCHAR_MAX : 0) << (CHAR_BIT - bit_shift)) & UCHAR_MAX);
...
void ammunition_unsigned_integer_shift_left(int size, const void *operand, int bits, void *result) {
    int byte_number;
    unsigned byte;
    unsigned carry;
    int bit_shift;
    ...
    if (bits < 0)
        ...
        byte_shift = bits / CHAR_BIT;
        bit_shift = bits % CHAR_BIT;
        for (byte_number = 0; byte_number < byte_shift && byte_number < size; byte_number++)
            if (((unsigned char *)operand)[byte_number] != 0) {
                ammunition_overflow_bit = 1;
                break;
            }
        if (byte_shift >= size)
            ...
            ammunition_memmove(result, (char *)operand + byte_shift, (size_t)(size - byte_shift));
            ammunition_memset((char *)result + (size - byte_shift), 0, (size_t)byte_shift);
            if (bit_shift == 0) return;
            for (byte_number = size - byte_shift - 1, carry = 0; byte_number >= 0; byte_number--) {
                byte = ((unsigned char *)result)[byte_number];
                ((unsigned char *)result)[byte_number] = carry | (byte << bit_shift);
...
void ammunition_integer_shift_left(int size, const void *operand, int bits, void *result) {
    int byte_number;
    unsigned byte;
    unsigned carry;
    int bit_shift;
    ...
    if (bits < 0)
        ...
        bit_shift = bits % CHAR_BIT;
        for (byte_number = 0; byte_number < byte_shift && byte_number < size; byte_number++)
            if (((unsigned char *)operand)[byte_number] != (operand_sign ? UCHAR_MAX : 0)) {
                ammunition_overflow_bit = 1;
                break;
            }
        if (byte_shift >= size)
            ...
            if (carry != ((unsigned)(operand_sign ? UCHAR_MAX : 0) >> (CHAR_BIT - bit_shift)))
                ammunition_overflow_bit = 1;
        }
        if (operand_sign != INTEGER_SIGN(result)) ammunition_overflow_bit = 1;
        if (ammunition_overflow_bit) ammunition_arithmetic_overflow_reaction();
    }
}
...
void ammunition_integer_or(int size, const void *op1, const void *op2, void *result) {
    int byte_number;

    for (byte_number = 0; byte_number < size; byte_number++) {
        ((unsigned char *)result)[byte_number] =
            ((unsigned char *)op1)[byte_number] | ((unsigned char *)op2)[byte_number];
    }
}
...
void ammunition_unsigned_integer_or(int size, const void *op1, const void *op2, void *result) {
    ammunition_integer_or(size, op1, op2, result);
}

/* This function makes bitwise `and' of two integers of given size. */

void ammunition_integer_and(int size, const void *op1, const void *op2, void *result) {
    int byte_number;
...
void ammunition_unsigned_integer_and(int size, const void *op1, const void *op2, void *result) {
...
void ammunition_integer_not(int size, const void *operand, void *result) {
    ...
    for (byte_number = 0; byte_number < size; byte_number++) {
        ((unsigned char *)result)[byte_number] = ((unsigned char *)operand)[byte_number] ^ UCHAR_MAX;
    }
}
...
void ammunition_unsigned_integer_not(int size, const void *operand, void *result) {
...
void ammunition_change_unsigned_integer_size(int operand_size, const void *operand, int result_size, void *result) {
    int operand_digit_number;

    ammunition_overflow_bit = 0;
    if (operand_size <= result_size) {
        ammunition_memmove((char *)result + result_size - operand_size, operand, (size_t)operand_size);
        ammunition_memset(result, 0, (size_t)(result_size - operand_size));
    } else {
        for (operand_digit_number = 0; operand_digit_number < operand_size - result_size; operand_digit_number++) {
            if (((unsigned char *)operand)[operand_digit_number] != 0) {
                ammunition_overflow_bit = 1;
                break;
            }
        }
        ammunition_memmove(result, (char *)operand + operand_size - result_size, (size_t)result_size);
    }
    if (ammunition_overflow_bit) ammunition_arithmetic_unsigned_overflow_reaction();
}
...
void ammunition_change_integer_size(int operand_size, const void *operand, int result_size, void *result) {
    int operand_digit_number;
    int operand_sign;
    ...
    if (operand_size <= result_size) {
        ammunition_memmove((char *)result + result_size - operand_size, operand, (size_t)operand_size);
        ammunition_memset(result, (operand_sign ? UCHAR_MAX : 0), (size_t)(result_size - operand_size));
    } else {
        for (operand_digit_number = 0; operand_digit_number < operand_size - result_size; operand_digit_number++) {
            if (((unsigned char *)operand)[operand_digit_number] != (operand_sign ? UCHAR_MAX : 0)) {
                ammunition_overflow_bit = 1;
                break;
            }
        }
...
char *ammunition_unsigned_integer_to_based_string(int size, const void *operand, int base, char *result) {
    int digit_num;
    int i;
    unsigned long long divisable;
    unsigned long long remainder;
    ...
    do {
        ...
        for (digit_num = 0, remainder = 0; digit_num < size; digit_num++) {
            ...
            if (operand_copy[digit_num] != 0) nonzero_flag = 1 /* TRUE */;
        }
        result[length++] = (unsigned char)(remainder < 10 ? '0' + remainder : 'a' + remainder - 10);
    } while (nonzero_flag);
    result[length] = '\0';
...
char *ammunition_unsigned_integer_to_string(int size, const void *operand, char *result) {
...
char *ammunition_integer_to_based_string(int size, const void *operand, int base, char *result) {
    unsigned char operand_copy[MAX_INTEGER_OPERAND_SIZE];

    if (!INTEGER_SIGN(operand)) return ammunition_unsigned_integer_to_based_string(size, operand, base, result);
    ammunition_memcpy(operand_copy, operand, (size_t)size);
...
int ammunition_add_digit_to_unsigned_integer_without_overflow_reaction(int size, void *operand, unsigned int digit) {
    int digit_num;
    unsigned int carry;
    unsigned int sum;
...
int ammunition_string_to_unsigned_integer_without_overflow_reaction(int size,
                                                                    const char *operand,
                                                                    void *result,
                                                                    char **first_nondigit) {
    int overflow_flag;
    ...
    for (overflow_flag = 0; ammunition_isdigit(*operand); operand++) {
        overflow_flag =
            overflow_flag || ammunition_multiply_unsigned_integer_by_digit_without_overflow_reaction(size, result, 10);
        overflow_flag = overflow_flag || ammunition_add_digit_to_unsigned_integer_without_overflow_reaction(
...
char *ammunition_unsigned_integer_from_string(int size, const char *operand, void *result) {
    char *first_nondigit;

    while (ammunition_isspace(*operand)) operand++;
    ammunition_overflow_bit =
        ammunition_string_to_unsigned_integer_without_overflow_reaction(size, operand, result, &first_nondigit);
    if (ammunition_overflow_bit) ammunition_arithmetic_unsigned_overflow_reaction();
...
char *ammunition_integer_from_string(int size, const char *operand, void *result) {
    ...
    ammunition_overflow_bit =
        ammunition_overflow_bit ||
        (unsigned_result_sign && (!negative_number_flag || INTEGER_SIGN(result) != unsigned_result_sign));
    if (ammunition_overflow_bit) ammunition_arithmetic_unsigned_overflow_reaction();
    return first_nondigit;
}
