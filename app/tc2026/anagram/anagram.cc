...
*/
void Anagram::anagram_ReadDict(void) {
    char *pch;
    char *pchBase;
    unsigned len;
    unsigned cWords = 0;
    ...
    for (i = 0; i < anagram_DICTWORDS; i++) {
        unsigned strlen = 0;
        while (anagram_dictionary[i][strlen] != 0) strlen++;
        len += strlen + 3;
    }
    ...
    for (i = 0; i < anagram_DICTWORDS; i++) {
        ...
        while (anagram_dictionary[i][index] != '\0') {
            if (anagram_isalpha(anagram_dictionary[i][index])) cLetters++;
...
int Anagram::anagram_ch2i(int ch) { return ch - 'a'; }

int Anagram::anagram_CompareFrequency(char *pch1, char *pch2) {
    return anagram_auGlobalFrequency[(*(int *)pch1)] < anagram_auGlobalFrequency[(*(int *)pch2)]    ? -1
           : anagram_auGlobalFrequency[(*(int *)pch1)] == anagram_auGlobalFrequency[(*(int *)pch2)] ? 0
                                                                                                    : 1;
}

void Anagram::anagram_Reset(void) {
...
void Anagram::anagram_BuildMask(char const *pchPhrase) {
    int i;
    int ch;
    ...
    for (i = 0; i < anagram_ALPHABET; i++) {
        if (anagram_alPhrase[i].uFrequency == 0) {
            ...
            for (cbtNeed = 1, qNeed = 1; anagram_alPhrase[i].uFrequency >= qNeed; cbtNeed++, qNeed <<= 1);
            if (cbtUsed + cbtNeed > anagram_MASK_BITS) cbtUsed = 0;
            anagram_alPhrase[i].uBits = qNeed - 1;
            if (cbtUsed) qNeed <<= cbtUsed;
            anagram_aqMainSign[iq] |= qNeed;
            anagram_aqMainMask[iq] |= (anagram_Quad)anagram_alPhrase[i].uFrequency << cbtUsed;
...
anagram_PWord Anagram::anagram_NewWord(void) {
    anagram_PWord pw;
...
anagram_PWord Anagram::anagram_NextWord(void) {
    anagram_PWord pw;
...
void Anagram::anagram_BuildWord(char *pchWord) {
    ...
    anagram_bzero((char *)cchFrequency, sizeof(unsigned char) * anagram_ALPHABET);

    /* Build frequency table */
    while ((i = *pch++) != '\0') {
        if (!anagram_isalpha(i)) continue;
        i = anagram_ch2i(anagram_tolower(i));
        if (++cchFrequency[i] > anagram_alPhrase[i].uFrequency) return;
        ++cchLength;
    }

    /* Update global count */
    for (i = 0; i < anagram_ALPHABET; i++) anagram_auGlobalFrequency[i] += cchFrequency[i];

    /* Create a Word structure and fill it in, including building the
       bitfield of frequencies. */
    pw = anagram_NextWord();
    anagram_bzero((char *)(pw->aqMask), sizeof(anagram_Quad) * anagram_MAX_QUADS);

    pw->pchWord = pchWord;
    pw->cchLength = cchLength;
    for (i = 0; i < anagram_ALPHABET; i++) {
        pw->aqMask[anagram_alPhrase[i].iq] |= (anagram_Quad)cchFrequency[i] << anagram_alPhrase[i].uShift;
    }
}

/* AddWords -- build the list of candidates */
void Anagram::anagram_AddWords(void) {
    char *pch = anagram_pchDictionary; /* walk through the dictionary */

    anagram_cpwCand = 0;

    while (*pch) {
        if ((pch[1] >= anagram_cchMinLength && pch[1] + anagram_cchMinLength <= anagram_cchPhraseLength) ||
...
void Anagram::anagram_DumpWords(void) {
    int i, j;
    int offset = 0;
...
void Anagram::anagram_FindAnagram(anagram_Quad *pqMask, anagram_PPWord ppwStart, int iLetter) {
...
void Anagram::anagram_SortCandidates(void) {
    int i;

    /* Sort the letters by frequency */
    for (i = 0; i < anagram_ALPHABET; i++) anagram_achByFrequency[i] = i;
...
int Anagram::run() {
    for (int i = 0; i < 3; i++) {
        ...
        anagram_AddWords();
        if (anagram_cpwCand == 0 || anagram_cchPhraseLength == 0) continue;
    ...
    for (int i = 0; i < 12; i++)
        if (answer[i] != anagram_buffer[i]) return 1;
