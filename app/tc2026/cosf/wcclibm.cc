...
float basicmath___ieee754_acosf(float x) {
    ...
    if (ix == 0x3f800000) { /* |x|==1 */
        if (hx > 0)
            ...
            return basicmath_pi + (float)2.0f * basicmath_pio2_lo; /* acos(-1)= pi */
    ...
    if (ix < 0x3f000000) {                                                  /* |x| < 0.5 */
        ...
        p = z *
            (basicmath_pS0 +
             z * (basicmath_pS1 + z * (basicmath_pS2 + z * (basicmath_pS3 + z * (basicmath_pS4 + z * basicmath_pS5)))));
...
float basicmath___ieee754_powf(float x, float y) {
    ...
    */
    ...
    if (iy > 0x4d000000) { /* if |y| > 2**27 */
        ...
        r = s2 * s2 *
            (basicmath_L1 +
             s2 * (basicmath_L2 + s2 * (basicmath_L3 + s2 * (basicmath_L4 + s2 * (basicmath_L5 + s2 * basicmath_L6)))));
        r += s_l * (s_h + s);
        s2 = s_h * s_h;
        t_h = (float)3.0f + s2 + r;
    ...
    if (((((u_int32_t)hx >> 31) - 1) | (yisint - 1)) == 0) s = -basicmath_one; /* (-ve)**(odd int) */
...
float basicmath___kernel_cosf(float x, float y) {
    ...
    z = x * x;
    r = z * (basicmath_C1 +
             z * (basicmath_C2 + z * (basicmath_C3 + z * (basicmath_C4 + z * (basicmath_C5 + z * basicmath_C6)))));
...
float basicmath___scalbnf(float x, int n) {
    ...
    k = k + n;
    if (n > 50000 || k > 0xfe) return basicmath_huge * basicmath___copysignf(basicmath_huge, x); /* overflow  */
    if (n < -50000) return basicmath_tiny * basicmath___copysignf(basicmath_tiny, x);            /*underflow*/
    if (k > 0) {                                                                                 /* normal result */
        SET_FLOAT_WORD(x, (ix & 0x807fffff) | (k << 23));
        return x;
    }
    if (k <= -25) return basicmath_tiny * basicmath___copysignf(basicmath_tiny, x); /*underflow*/
