...
class G723Enc {
   public:
    G723Enc() {
        ...
        for (i = 0; i < 256; i++) g723_enc_INPUT[i] += x;
    }
    ...
    inline int run() {
        ...
        while (resid) resid = g723_enc_pack_output(0, enc_bits);

        int check_sum = 0;

        for (int i = 0; i < 256; i++) check_sum += g723_enc_OUTPUT[i];
...
*/
    ...
    */
    void g723_enc_update(int code_size,                      /* distinguish 723_40 with others */
                         ...
                         struct g723_enc_state_t *state_ptr) /* coder state pointer */
    {
        ...
        */
        ...
        if (tr == 1)
            ...
            state_ptr->ap += (0x200 - state_ptr->ap) >> 4;
        else if (g723_enc_abs((state_ptr->dms << 2) - state_ptr->dml) >= (state_ptr->dml >> 3))
            state_ptr->ap += (0x200 - state_ptr->ap) >> 4;
        else
    ...
    */
    int g723_enc_g723_24_encoder(int sl, int in_coding, struct g723_enc_state_t *state_ptr) {
   ...
   private:
    ...
    unsigned int g723_enc_INPUT[256] = {
        51, 17, 31, 53, 95, 17, 70, 22, 49, 12, 8,  39, 28, 37, 99, 54, 77, 65, 77, 78, 83, 15, 63, 31, 35, 92,
        52, 40, 61, 79, 94, 87, 87, 68, 76, 58, 39, 35, 20, 83, 42, 46, 98, 12, 21, 96, 74, 41, 78, 76, 96, 2,
        32, 76, 24, 59, 4,  96, 32, 5,  44, 92, 57, 12, 57, 25, 50, 23, 48, 41, 88, 43, 36, 38, 4,  16, 52, 70,
        9,  40, 78, 24, 34, 23, 30, 30, 89, 3,  65, 40, 68, 73, 94, 23, 84, 97, 78, 43, 68, 81, 16, 28, 13, 87,
        75, 21, 14, 29, 81, 22, 56, 72, 19, 99, 25, 43, 76, 86, 90, 98, 39, 43, 12, 46, 24, 99, 65, 61, 24, 45,
        79, 7,  48, 15, 24, 95, 62, 99, 48, 80, 75, 38, 48, 53, 9,  60, 35, 14, 78, 71, 45, 71, 9,  97, 55, 74,
        58, 64, 78, 18, 30, 28, 69, 29, 57, 42, 30, 44, 57, 49, 61, 42, 13, 25, 3,  98, 11, 38, 65, 35, 55, 36,
        57, 48, 16, 62, 17, 56, 29, 88, 84, 85, 90, 60, 54, 16, 66, 69, 26, 10, 82, 19, 42, 35, 84, 13, 26, 17,
        48, 38, 50, 50, 35, 53, 12, 52, 61, 74, 56, 34, 80, 59, 26, 67, 55, 79, 89, 89, 6,  80, 91, 65, 16, 30,
        16, 28, 85, 54, 3,  20, 2,  36, 62, 52, 55, 15, 83, 3,  2,  38, 62, 2,  63, 92, 37, 73};

    unsigned int g723_enc_OUTPUT[256];

    short g723_enc_power2[15] =
    ...
    */
    ...
    /*
       ...
       magnitude values.
    */
    short g723_enc_dqlntab[16] = {-2048, 4, 135, 213, 273, 323, 373, 425, 425, 373, 323, 273, 213, 135, 4, -2048};

    /* Maps G.721 code word to log of scale factor multiplier. */
    short g723_enc_witab[16] = {-12, 18, 41, 64, 112, 198, 355, 1122, 1122, 355, 198, 112, 64, 41, 18, -12};
    /*
