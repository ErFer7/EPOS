...
class HuffEnc {
   public:
    ...
    inline int run() {
        ...
        for (int i = 0; i < huff_enc_encoded_len; i++) {
...
*/
    ...
    void huff_enc_beginning_of_data() { huff_enc_input_pos = 0; }

    int huff_enc_end_of_data() { return huff_enc_input_pos >= huff_enc_plaintext_len; }

    int huff_enc_read_byte() { return huff_enc_plaintext[huff_enc_input_pos++]; }

    void huff_enc_write_byte(char ch) { huff_enc_output[huff_enc_output_pos++] = ch; }
    ...
    */
    {
        ...
        for (bit_indice = 1; bit_indice <= bin_val.bits_nb; bit_indice++) {
            ...
            huff_enc_val_to_write = (huff_enc_val_to_write << 1) | ((bin_val.bits[pos_byte] >> bin_pos) & 1);
    ...
    */
    {
        if (huff_enc_byte_nb_to_write) huff_enc_write_byte(huff_enc_val_to_write << (8 - huff_enc_byte_nb_to_write));
    }
    ...
    */
    {
        ...
        return ((*tree2)->weight ^ (*tree1)->weight) ? (((*tree2)->weight < (*tree1)->weight) ? -1 : 1) : 0;
    }
    ...
    char *huff_enc_pivot(char *a, unsigned long n, unsigned long es) {
        ...
        if (huff_enc_weighhuff_enc_t_tree_comp(pi, pj) < 0) {
            if (huff_enc_weighhuff_enc_t_tree_comp(pi, pk) < 0) {
                if (huff_enc_weighhuff_enc_t_tree_comp(pj, pk) < 0) return pj;
                return pk;
            }
            return pi;
        }
        if (huff_enc_weighhuff_enc_t_tree_comp(pj, pk) < 0) {
            if (huff_enc_weighhuff_enc_t_tree_comp(pi, pk) < 0) return pi;
            return pk;
        }
        return pj;
    }
    ...
    */
    {
        ...
        if (!huff_enc_end_of_data()) {
            ...
            while (i > 0) {
                ...
                huff_enc_qsort((char *)occurrences_table, i + 1, sizeof(huff_enc_t_tree *));
    ...
    */
    {
        ...
        if (tree->byte == 257) {
            ...
            /* The sub-trees on left begin with an bit set to 1 */
            {
                ...
                for (i = 31; i > 0; i--) code_val->bits[i] = (code_val->bits[i] << 1) | (code_val->bits[i - 1] >> 7);
    ...
    void huff_enc_create_codes_table(huff_enc_t_tree *tree, huff_enc_t_bin_val codes_table[257])
    ...
    */
    {
        ...
        for (i = 0; i < 32; i++) code_val.bits[i] = 0;
        code_val.bits_nb = 0;
        for (j = 0; j < 257; j++) {
            for (i = 0; i < 32; i++) codes_table[j].bits[i] = 0;
   ...
   private:
    int huff_enc_input_pos;
    int huff_enc_output_pos;
    unsigned char huff_enc_output[1024];
    unsigned char huff_enc_byte_nb_to_write = 0;
    unsigned char huff_enc_val_to_write = 0;
    inline static const char *huff_enc_plaintext =
    ...
    inline static unsigned char huff_enc_encoded[huff_enc_encoded_len] = {
        ...
        201, 68,  64,  114, 53,  17,  42,  37,  195, 128, 212, 116, 194, 41,  98,  52,  51,  12,  132, 112, 244,
        3,   36,  33,  52,  39,  135, 164, 33,  62,  156, 87,  14,  110, 22,  87,  50,  85,  198, 99,  142, 140,
        194, 81,  78,  158, 84,  129, 254, 129, 248, 110, 179, 159, 192, 145, 133, 184, 184, 28,  210, 96,  146,
        73,  10,  226, 21,  83,  152, 74,  13,  111, 132, 199, 202, 219, 241, 74,  193, 167, 105, 222, 31,  147,
        6,   55,  31,  129, 40,  232, 52,  153, 160, 148, 18,  36,  197, 45,  216, 202, 86,  30,  31,  177, 90,
        133, 138, 248, 23,  81,  195, 160, 100, 215, 93,  50,  185, 225, 251, 23,  6,   230, 225, 229, 112, 71,
        80,  96,  141, 205, 176, 230, 85,  196, 9,   24,  93,  90,  121, 225, 76,  68,  152, 63,  25,  107, 140,
        101, 204, 214, 77,  26,  194, 96,  18,  48,  77,  210, 137, 1,   253, 4,   230, 248, 56,  240, 224, 111,
        163, 95,  10,  12,  223, 7,   234, 167, 129, 40,  36,  96,  135, 125, 245, 250, 2,   198, 120, 127, 0,
        145, 133, 213, 167, 135, 149, 195, 67,  235, 108, 9,   24,  87,  17,  102, 152, 37,  4,   222, 131, 188,
        144, 73,  36,  128, 73,  20,  81,  152, 177, 133, 248, 28,  165, 131, 120, 127, 240, 242, 184, 104, 125,
        109, 129, 35,  30,  4,   145, 65,  202, 88,  9,   138, 103, 44,  205, 100, 167, 24,  152, 11,  24,  51,
        37,  66,  9,   24,  31,  174, 202, 212, 49,  152, 18,  96,  155, 208, 119, 146, 45,  97,  48,  56,  28,
        194, 90,  224, 204, 144, 232, 176, 36,  96,  126, 187, 43,  83,  12,  121, 129, 209, 96,  197, 35,  2,
        54,  176, 249, 92,  208, 204, 145, 188, 41,  170, 180, 71,  16,  36,  96,  126, 187, 43,  83,  19,  0,
        145, 129, 100, 209, 15,  43,  135, 55,  6,   238, 180, 194, 90,  17,  229, 115, 21,  168, 251, 140, 131,
