...
class Iir {
   public:
    Iir() {
        ...
        for (f = 0; f < 5 * 4; f++) iir_coefficients[f] = 7;

        for (f = 0; f < 2 * 4; f++) iir_wi[f] = 0;
        ...
        */
        ...
        for (i = 0; i < sizeof(iir_coefficients); ++i, ++p) *p ^= bitmask;

        p = (unsigned char *)&iir_wi[0];

        for (i = 0; i < sizeof(iir_wi); ++i, ++p) *p ^= bitmask;
    }
   ...
   private:
    inline int iir_return(void) {
        ...
        for (f = 0; f < 2 * 4; f++) checksum += iir_wi[f];
