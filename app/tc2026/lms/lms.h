...
class Lms {
   public:
    ...
    inline int run() {
        ...
        for (int i = 0; i < N; i++) {
            lms_output[i] = lms_calc(lms_input[i], lms_input[i + 1], b, L, 0.02 / (L + 1), 0.01, history, &sigma);
        }
   ...
   private:
    double lms_sinus(int i) {
        int s = i % (4 * SAMPLING);
        if (s >= (2 * SAMPLING)) return -lms_sintab[(s > 3 * SAMPLING) ? (4 * SAMPLING - s) : (s - 2 * SAMPLING)];
        return lms_sintab[(s > SAMPLING) ? (2 * SAMPLING - s) : s];
    }

    float lms_calc(float x, float d, float b[], int l, float mu, float alpha, float history[], float *sigma) {
        int i;

        // shift history

        for (i = l; i >= 1; i--) history[i] = history[i - 1];
        history[0] = x;
        ...
        *sigma = alpha * x * x + (1 - alpha) * (*sigma);
        ...
        for (i = 0; i <= l; i++) y += b[i] * history[i];

        // update coefficients
        float e = mu * (d - y) / (*sigma);

        for (i = 0; i <= l; i++) b[i] += e * history[i];
