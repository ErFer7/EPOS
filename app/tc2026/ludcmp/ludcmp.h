...
class LudCmp {
   public:
    ...
    inline int run() {
        ...
        for (int i = 0; i <= n; i++) checksum += ludcmp_x[i];
   ...
   private:
    ...
    int ludcmp_test(int n, double eps) {
        ...
        if (n > 99 || eps <= 0) return (999);

        for (i = 0; i < n; i++) {
            if (ludcmp_fabs(ludcmp_a[i][i]) <= eps) return (1);

            for (j = i + 1; j <= n; j++) {
                w = ludcmp_a[j][i];

                if (i != 0) {
                    for (k = 0; k < i; k++) w -= ludcmp_a[j][k] * ludcmp_a[k][i];
                }
            ...
            for (j = i + 1; j <= n; j++) {
                ...
                for (k = 0; k <= i; k++) w -= ludcmp_a[i + 1][k] * ludcmp_a[k][j];
        ...
        for (i = 1; i <= n; i++) {
            ...
            for (j = 0; j < i; j++) w -= ludcmp_a[i][j] * y[j];
        ...
        for (i = n - 1; i >= 0; i--) {
            ...
            for (j = i + 1; j <= n; j++) w -= ludcmp_a[i][j] * ludcmp_x[j];
