...
class Matrix1 {
   public:
    Matrix1() { matrix1_pin_down(&matrix1_A[0], &matrix1_B[0], &matrix1_C[0]); }
    ...
    inline int run() {
        ...
        for (int i = 0; i < X * Z; i++) checksum += matrix1_C[i];
   ...
   private:
    void matrix1_pin_down(int A[], int B[], int C[]) {
        ...
        for (i = 0; i < X * Y; i++) A[i] = x;

        for (i = 0; i < Y * Z; i++) B[i] = x;

        for (i = 0; i < X * Z; i++) C[i] = 0;
    }
