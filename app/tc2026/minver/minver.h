...
class Minver {
   public:
    ...
    inline int run() {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) minver_aa[i][j] = minver_a[i][j];
        }

        minver_minver(3, eps);

        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) minver_a_i[i][j] = minver_a[i][j];
        }
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) check_sum += minver_a_i[i][j];
        }
   ...
   private:
    ...
    int minver_mmul(int row_a, int col_a, int row_b, int col_b) {
        ...
        for (i = 0; i < row_c; i++) {
            for (j = 0; j < col_c; j++) {
                ...
                for (k = 0; k < row_b; k++) w += minver_a[i][k] * minver_b[k][j];
    ...
    int minver_minver(int side, double eps) {
        ...
        if (side < 2 || side > 500 || eps <= 0.0) return (999);
        w1 = 1.0;

        for (i = 0; i < side; i++) work[i] = i;
        ...
        for (k = 0; k < side; k++) {
            ...
            for (i = 0; i < side; i++) minver_a[k][i] /= pivot;
