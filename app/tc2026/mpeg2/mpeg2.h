...
class Mpeg2 {
   ...
   public:
    Mpeg2()
        ...
        /*
          Manually copy from the read-only template to the mutable working frame,
          applying the volatile XOR-bitmask at the same time to avoid a second pass.
        */
        for (i = 0; i < mpeg2_oldorgframe_size; ++i) {
            *dst++ = *src++ ^ bitmask;
        }
    }
    ...
    int mpeg2_return(void) {
        ...
        for (i = 0; i < 352; i++) {
            ...
            for (; j < 2; j++) {
                ...
                for (; k < 2; k++) {
                    l = 0;
                    for (; l < 2; l++) checksum += mpeg2_mbinfo[i].MV[j][k][l];
                }
            }
        }
...
*/
    void mpeg2_motion_estimation(unsigned char *oldorg,
        ...
        for (j = 0; j < mpeg2_height2; j += 16) {
            ...
            for (; i < mpeg2_width; i += 16) {
                if (mpeg2_pict_struct == 3)
                    mpeg2_frame_ME(oldorg, neworg, oldref, newref, cur, i, j, sxf, syf, sxb, syb, mbi);
                else
                    mpeg2_field_ME(oldorg,
                                   neworg,
                                   oldref,
                                   newref,
                                   cur,
    ...
    void mpeg2_frame_ME(unsigned char *oldorg,
                        unsigned char *neworg,
                        unsigned char *oldref,
                        unsigned char *newref,
                        unsigned char *cur,
        ...
        if (mpeg2_pict_type == 1)
            ...
            if (mpeg2_frame_pred_dct) {
                dmc = mpeg2_fullsearch(
                    oldorg, oldref, mb, mpeg2_width, i, j, sxf, syf, 16, mpeg2_width, mpeg2_height, &imin, &jmin);
                vmc = mpeg2_dist2(
                    oldref + (imin >> 1) + mpeg2_width * (jmin >> 1), mb, mpeg2_width, imin & 1, jmin & 1, 16);
                mbi->motion_type = 2;
            } else {
                mpeg2_frame_estimate(oldorg,
                                     oldref,
                                     mb,
                                     i,
                                     j,
                ...
                if (mpeg2_M == 1)
                    mpeg2_dpframe_estimate(
                        oldref, mb, i, j >> 1, imins, jmins, &imindp, &jmindp, &imindmv, &jmindmv, &dmc_dp, &vmc_dp);
                ...
                if ((mpeg2_M == 1) && (dmc_dp < dmc) && (dmc_dp < dmcfield)) {
                    ...
                    dmc = dmcfield;
                    vmc = mpeg2_dist2(
                        oldref + (tsel ? mpeg2_width : 0) + (imint >> 1) + (mpeg2_width << 1) * (jmint >> 1),
                        mb,
                        mpeg2_width << 1,
                        imint & 1,
                        jmint & 1,
                        8);
                    vmc += mpeg2_dist2(
                        oldref + (bsel ? mpeg2_width : 0) + (iminb >> 1) + (mpeg2_width << 1) * (jminb >> 1),
                        mb + mpeg2_width,
                        mpeg2_width << 1,
                        iminb & 1,
                        jminb & 1,
            ...
            */
            if ((vmc > var) && (vmc >= 9 * 256))
                ...
                */
                v0 = mpeg2_dist2(oldref + i + mpeg2_width * j, mb, mpeg2_width, 0, 0, 16);
            ...
            if (mpeg2_frame_pred_dct) {
                /* forward */
                dmcf = mpeg2_fullsearch(
                    oldorg, oldref, mb, mpeg2_width, i, j, sxf, syf, 16, mpeg2_width, mpeg2_height, &iminf, &jminf);
                vmcf = mpeg2_dist2(
                    oldref + (iminf >> 1) + mpeg2_width * (jminf >> 1), mb, mpeg2_width, iminf & 1, jminf & 1, 16);

                /* backward */
                dmcr = mpeg2_fullsearch(
                    neworg, newref, mb, mpeg2_width, i, j, sxb, syb, 16, mpeg2_width, mpeg2_height, &iminr, &jminr);
                vmcr = mpeg2_dist2(
                    newref + (iminr >> 1) + mpeg2_width * (jminr >> 1), mb, mpeg2_width, iminr & 1, jminr & 1, 16);

                /* interpolated (bidirectional) */
                vmci = mpeg2_bdist2(oldref + (iminf >> 1) + mpeg2_width * (jminf >> 1),
                ...
                */
                ...
                /* frame */
                dmci = mpeg2_bdist1(oldref + (iminf >> 1) + mpeg2_width * (jminf >> 1),
                                    newref + (iminr >> 1) + mpeg2_width * (jminr >> 1),
                                    mb,
                                    mpeg2_width,
                                    iminf & 1,
                ...
                /* top field */
                dmcfieldi = mpeg2_bdist1(
                    oldref + (imintf >> 1) + (tself ? mpeg2_width : 0) + (mpeg2_width << 1) * (jmintf >> 1),
                    newref + (imintr >> 1) + (tselr ? mpeg2_width : 0) + (mpeg2_width << 1) * (jmintr >> 1),
                    mb,
                    mpeg2_width << 1,
                    imintf & 1,
                    jmintf & 1,
                ...
                /* bottom field */
                dmcfieldi += mpeg2_bdist1(
                    oldref + (iminbf >> 1) + (bself ? mpeg2_width : 0) + (mpeg2_width << 1) * (jminbf >> 1),
                    newref + (iminbr >> 1) + (bselr ? mpeg2_width : 0) + (mpeg2_width << 1) * (jminbr >> 1),
                    mb + mpeg2_width,
                    mpeg2_width << 1,
                    iminbf & 1,
                    jminbf & 1,
                ...
                */
                if ((dmci < dmcfieldi) && (dmci < dmcf) && (dmci < dmcfieldf) && (dmci < dmcr) && (dmci < dmcfieldr)) {
                    ...
                    mbi->mb_type = 8 | 4;
                    mbi->motion_type = 2;
                    vmc = mpeg2_bdist2(oldref + (iminf >> 1) + mpeg2_width * (jminf >> 1),
                                       newref + (iminr >> 1) + mpeg2_width * (jminr >> 1),
                                       mb,
                                       mpeg2_width,
                                       iminf & 1,
                    ...
                    if ((dmcfieldi < dmcf) && (dmcfieldi < dmcfieldf) && (dmcfieldi < dmcr) &&
                        (dmcfieldi < dmcfieldr)) {
                    /* field, interpolated */
                    mbi->mb_type = 8 | 4;
                    mbi->motion_type = 1;
                    ...
                    mbi->motion_type = 1;
                    vmc = mpeg2_dist2(
                        oldref + (tself ? mpeg2_width : 0) + (imintf >> 1) + (mpeg2_width << 1) * (jmintf >> 1),
                        mb,
                        mpeg2_width << 1,
                        imintf & 1,
                        jmintf & 1,
                        8);
                    vmc += mpeg2_dist2(
                        oldref + (bself ? mpeg2_width : 0) + (iminbf >> 1) + (mpeg2_width << 1) * (jminbf >> 1),
                        mb + mpeg2_width,
                        mpeg2_width << 1,
                        iminbf & 1,
                        jminbf & 1,
                    ...
                    mbi->motion_type = 1;
                    vmc = mpeg2_dist2(
                        newref + (tselr ? mpeg2_width : 0) + (imintr >> 1) + (mpeg2_width << 1) * (jmintr >> 1),
                        mb,
                        mpeg2_width << 1,
                        imintr & 1,
                        jmintr & 1,
                        8);
                    vmc += mpeg2_dist2(
                        newref + (bselr ? mpeg2_width : 0) + (iminbr >> 1) + (mpeg2_width << 1) * (jminbr >> 1),
                        mb + mpeg2_width,
                        mpeg2_width << 1,
                        iminbr & 1,
                        jminbr & 1,
    ...
    */
    void mpeg2_field_ME(unsigned char *oldorg,
                        unsigned char *neworg,
                        unsigned char *oldref,
                        unsigned char *newref,
                        unsigned char *cur,
        ...
        if (mpeg2_pict_struct == 2) mb += mpeg2_width;
        ...
        if (mpeg2_pict_type == 1)
            ...
            if ((mpeg2_M == 1) && !ipflag) /* generic condition which permits Dual Prime */
                mpeg2_dpfield_estimate(topref, botref, mb, i, j, imins, jmins, &imindmv, &jmindmv, &dmc_dp, &vmc_dp);

            /* select between dual prime, field and 16x8 prediction */
            if ((mpeg2_M == 1) && !ipflag && (dmc_dp < dmc8) && (dmc_dp < dmcfield)) {
                ...
                /* upper half block */
                vmc = mpeg2_dist2(
                    (sel8u ? botref : topref) + (imin8u >> 1) + w2 * (jmin8u >> 1), mb, w2, imin8u & 1, jmin8u & 1, 8);
                /* lower half block */
                vmc += mpeg2_dist2((sel8l ? botref : topref) + (imin8l >> 1) + w2 * (jmin8l >> 1),
                                   mb + 8 * w2,
                                   w2,
                                   imin8l & 1,
                                   jmin8l & 1,
                ...
                mbi->motion_type = 1;
                vmc = mpeg2_dist2(
                    (sel ? botref : topref) + (imin >> 1) + w2 * (jmin >> 1), mb, w2, imin & 1, jmin & 1, 16);
            }
            ...
            if ((vmc > var) && (vmc >= 9 * 256))
                ...
                */
                if (!ipflag)
                    v0 = mpeg2_dist2(((mpeg2_pict_struct == 2) ? botref : topref) + i + w2 * j, mb, w2, 0, 0, 16);
            ...
            /* field */
            dmcfieldi = mpeg2_bdist1(oldref + (self ? mpeg2_width : 0) + (iminf >> 1) + w2 * (jminf >> 1),
                                     newref + (selr ? mpeg2_width : 0) + (iminr >> 1) + w2 * (jminr >> 1),
                                     mb,
                                     w2,
                                     iminf & 1,
                                     jminf & 1,
            ...
            dmc8i = mpeg2_bdist1(oldref + (sel8uf ? mpeg2_width : 0) + (imin8uf >> 1) + w2 * (jmin8uf >> 1),
                                 newref + (sel8ur ? mpeg2_width : 0) + (imin8ur >> 1) + w2 * (jmin8ur >> 1),
                                 mb,
                                 w2,
                                 imin8uf & 1,
                                 jmin8uf & 1,
            ...
            dmc8i += mpeg2_bdist1(oldref + (sel8lf ? mpeg2_width : 0) + (imin8lf >> 1) + w2 * (jmin8lf >> 1),
                                  newref + (sel8lr ? mpeg2_width : 0) + (imin8lr >> 1) + w2 * (jmin8lr >> 1),
                                  mb + 8 * w2,
                                  w2,
                                  imin8lf & 1,
                                  jmin8lf & 1,
            ...
            if ((dmcfieldi < dmc8i) && (dmcfieldi < dmcfieldf) && (dmcfieldi < dmc8f) && (dmcfieldi < dmcfieldr) &&
                (dmcfieldi < dmc8r)) {
                /* field, interpolated */
                mbi->mb_type = 8 | 5;
                mbi->motion_type = 1;
                vmc = mpeg2_bdist2(oldref + (self ? mpeg2_width : 0) + (iminf >> 1) + w2 * (jminf >> 1),
                                   newref + (selr ? mpeg2_width : 0) + (iminr >> 1) + w2 * (jminr >> 1),
                                   mb,
                                   w2,
                                   iminf & 1,
                ...
                if ((dmc8i < dmcfieldf) && (dmc8i < dmc8f) && (dmc8i < dmcfieldr) && (dmc8i < dmc8r)) {
                /* 16x8, interpolated */
                mbi->mb_type = 8 | 4;
                mbi->motion_type = 2;

                /* upper half block */
                vmc = mpeg2_bdist2(oldref + (sel8uf ? mpeg2_width : 0) + (imin8uf >> 1) + w2 * (jmin8uf >> 1),
                                   newref + (sel8ur ? mpeg2_width : 0) + (imin8ur >> 1) + w2 * (jmin8ur >> 1),
                                   mb,
                                   w2,
                                   imin8uf & 1,
                ...
                /* lower half block */
                vmc += mpeg2_bdist2(oldref + (sel8lf ? mpeg2_width : 0) + (imin8lf >> 1) + w2 * (jmin8lf >> 1),
                                    newref + (sel8lr ? mpeg2_width : 0) + (imin8lr >> 1) + w2 * (jmin8lr >> 1),
                                    mb + 8 * w2,
                                    w2,
                                    imin8lf & 1,
                ...
                if ((dmcfieldf < dmc8f) && (dmcfieldf < dmcfieldr) && (dmcfieldf < dmc8r)) {
                /* field, forward */
                mbi->mb_type = 8;
                mbi->motion_type = 1;
                vmc = mpeg2_dist2(oldref + (self ? mpeg2_width : 0) + (iminf >> 1) + w2 * (jminf >> 1),
                                  mb,
                                  w2,
                                  iminf & 1,
                                  jminf & 1,
                ...
                /* upper half block */
                vmc = mpeg2_dist2(oldref + (sel8uf ? mpeg2_width : 0) + (imin8uf >> 1) + w2 * (jmin8uf >> 1),
                                  mb,
                                  w2,
                                  imin8uf & 1,
                                  jmin8uf & 1,
                ...
                /* lower half block */
                vmc += mpeg2_dist2(oldref + (sel8lf ? mpeg2_width : 0) + (imin8lf >> 1) + w2 * (jmin8lf >> 1),
                                   mb + 8 * w2,
                                   w2,
                                   imin8lf & 1,
                                   jmin8lf & 1,
                ...
                mbi->mb_type = 4;
                mbi->motion_type = 1;
                vmc = mpeg2_dist2(newref + (selr ? mpeg2_width : 0) + (iminr >> 1) + w2 * (jminr >> 1),
                                  mb,
                                  w2,
                                  iminr & 1,
                                  jminr & 1,
                ...
                /* upper half block */
                vmc = mpeg2_dist2(newref + (sel8ur ? mpeg2_width : 0) + (imin8ur >> 1) + w2 * (jmin8ur >> 1),
                                  mb,
                                  w2,
                                  imin8ur & 1,
                                  jmin8ur & 1,
                ...
                /* lower half block */
                vmc += mpeg2_dist2(newref + (sel8lr ? mpeg2_width : 0) + (imin8lr >> 1) + w2 * (jmin8lr >> 1),
                                   mb + 8 * w2,
                                   w2,
                                   imin8lr & 1,
                                   jmin8lr & 1,
    ...
    */
    void mpeg2_frame_estimate(unsigned char *org,
                              unsigned char *ref,
                              unsigned char *mb,
                              int i,
                              int j,
        ...
        /* frame prediction */
        *dframep =
            mpeg2_fullsearch(org, ref, mb, mpeg2_width, i, j, sx, sy, 16, mpeg2_width, mpeg2_height, iminp, jminp);

        /* predict top field from top field */
        dt = mpeg2_fullsearch(
            org, ref, mb, mpeg2_width << 1, i, j >> 1, sx, sy >> 1, 8, mpeg2_width, mpeg2_height >> 1, &imint, &jmint);

        /* predict top field from bottom field */
        db = mpeg2_fullsearch(org + mpeg2_width,
                              ref + mpeg2_width,
                              mb,
                              mpeg2_width << 1,
                              i,
    ...
    /*
      ...
      iminsp,jminsp,dsp: location and distance of best same parity field
                         ...
                         ipflag==0)
    */
    void mpeg2_field_estimate(unsigned char *toporg,
    ...
    void mpeg2_dpframe_estimate(unsigned char *ref,
                                unsigned char *mb,
                                int i,
                                int j,
                                int iminf[2][2],
        ...
        */
        ...
        for (pref = 0; pref < 2; pref++) {
            ...
            for (; ppred < 2; ppred++) {
                ...
                */
                ...
                if ((is >= 0) && (is <= (mpeg2_width - 16) << 1) && (js >= 0) && (js <= (mpeg2_height - 16))) {
                    for (delta_y = -1; delta_y <= 1; delta_y++) {
                        ...
                        for (; delta_x <= 1; delta_x++) {
                            ...
                            if ((it >= 0) && (it <= (mpeg2_width - 16) << 1) && (jt >= 0) &&
                                (jt <= (mpeg2_height - 16)) && (ib >= 0) && (ib <= (mpeg2_width - 16) << 1) &&
                                (jb >= 0) && (jb <= (mpeg2_height - 16))) {
                                /* compute prediction error */
                                local_dist =
                                    mpeg2_bdist2(ref + (is >> 1) + (mpeg2_width << 1) * (js >> 1),
                                                 ref + mpeg2_width + (it >> 1) + (mpeg2_width << 1) * (jt >> 1),
                                                 ...
                                                 8);     /* block height */
                                local_dist +=
                                    mpeg2_bdist2(ref + mpeg2_width + (is >> 1) + (mpeg2_width << 1) * (js >> 1),
        ...
        local_dist = mpeg2_bdist1(ref + (imins >> 1) + (mpeg2_width << 1) * (jmins >> 1),
                                  ref + mpeg2_width + (imint >> 1) + (mpeg2_width << 1) * (jmint >> 1),
                                  mb,
                                  mpeg2_width << 1,
                                  imins & 1,
                                  jmins & 1,
                                  ...
                                  jmint & 1,
                                  8);
        local_dist += mpeg2_bdist1(ref + mpeg2_width + (imins >> 1) + (mpeg2_width << 1) * (jmins >> 1),
    ...
    void mpeg2_dpfield_estimate(unsigned char *topref,
                                unsigned char *botref,
                                unsigned char *mb,
                                int i,
                                int j,
        ...
        */
        ...
        for (delta_y = -1; delta_y <= 1; delta_y++) {
            ...
            for (; delta_x <= 1; delta_x++) {
                ...
                if ((io >= 0) && (io <= (mpeg2_width - 16) << 1) && (jo >= 0) && (jo <= (mpeg2_height2 - 16) << 1)) {
                    /* compute prediction error */
                    local_dist = mpeg2_bdist2(sameref + (imins >> 1) + mpeg2_width2 * (jmins >> 1),
        ...
        /* Compute L1 error for decision purposes */
        *dmcp = mpeg2_bdist1(sameref + (imins >> 1) + mpeg2_width2 * (jmins >> 1),
    ...
    */
    int mpeg2_fullsearch(unsigned char *org,
                         unsigned char *ref,
                         unsigned char *blk,
                         int lx,
                         int i0,
        ...
        for (j = jlow; j <= jhigh; j++) {
            i = ilow;
            for (; i <= ihigh; i++) {
                d = mpeg2_dist1(ref + (i >> 1) + lx * (j >> 1), blk, lx, i & 1, j & 1, h, dmin);
    ...
    */
    int mpeg2_dist1(unsigned char *blk1, unsigned char *blk2, int lx, int hx, int hy, int h, int distlim) {
        ...
        if (!hx && !hy) {
            for (j = 0; j < h; j++) {
                ...
                if ((v = p1[2] - p2[2]) < 0) v = -v;
                s += v;

                if ((v = p1[3] - p2[3]) < 0) v = -v;
                s += v;

                if ((v = p1[4] - p2[4]) < 0) v = -v;
                s += v;

                if ((v = p1[5] - p2[5]) < 0) v = -v;
                s += v;

                if ((v = p1[6] - p2[6]) < 0) v = -v;
                s += v;

                if ((v = p1[7] - p2[7]) < 0) v = -v;
                s += v;

                if ((v = p1[8] - p2[8]) < 0) v = -v;
                s += v;

                if ((v = p1[9] - p2[9]) < 0) v = -v;
                s += v;

                if ((v = p1[10] - p2[10]) < 0) v = -v;
                s += v;

                if ((v = p1[11] - p2[11]) < 0) v = -v;
                s += v;

                if ((v = p1[12] - p2[12]) < 0) v = -v;
                s += v;

                if ((v = p1[13] - p2[13]) < 0) v = -v;
                s += v;

                if ((v = p1[14] - p2[14]) < 0) v = -v;
                s += v;

                if ((v = p1[15] - p2[15]) < 0) v = -v;
                s += v;
    ...
    /*
      ...
      h:         height of block (usually 8 or 16)
    */
    int mpeg2_dist2(unsigned char *blk1, unsigned char *blk2, int lx, int hx, int hy, int h) {
        unsigned char *p1, *p1a, *p2;
        int i, j;
        int s, v;
    ...
    */
    int mpeg2_bdist1(unsigned char *pf,
                     unsigned char *pb,
                     unsigned char *p2,
                     int lx,
                     int hxf,
        ...
        for (j = 0; j < h; j++) {
            ...
            for (; i < 16; i++) {
                v = ((((unsigned int)(*pf++ + *pfa++ + *pfb++ + *pfc++ + 2) >> 2) +
                      ((unsigned int)(*pb++ + *pba++ + *pbb++ + *pbc++ + 2) >> 2) + 1) >>
                     1) -
                    *p2++;
                if (v >= 0)
                    s += v;
    ...
    /*
      ...
      lx: distance (in bytes) of vertically adjacent pels in p2,pf,pb
    */
    int mpeg2_bdist2(unsigned char *pf,
                     unsigned char *pb,
                     unsigned char *p2,
                     int lx,
                     int hxf,
