...
class Ndes {
   public:
    Ndes() {
        unsigned int i;
        const char ndes_ipc1_tmp[57] = {0,  57, 49, 41, 33, 25, 17, 9,  1,  58, 50, 42, 34, 26, 18, 10, 2,  59, 51,
                                        43, 35, 27, 19, 11, 3,  60, 52, 44, 36, 63, 55, 47, 39, 31, 23, 15, 7,  62,
                                        54, 46, 38, 30, 22, 14, 6,  61, 53, 45, 37, 29, 21, 13, 5,  28, 20, 12, 4};
        const char ndes_ipc2_tmp[49] = {0,  14, 17, 11, 24, 1,  5,  3,  28, 15, 6,  21, 10, 23, 19, 12, 4,
                                        26, 8,  16, 7,  27, 20, 13, 2,  41, 52, 31, 37, 47, 55, 30, 40, 51,
                                        45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32};
        for (i = 0; i < 57; i++) ndes_ipc1[i] = ndes_ipc1_tmp[i];
        for (i = 0; i < 49; i++) ndes_ipc2[i] = ndes_ipc2_tmp[i];
   ...
   private:
    void ndes_des(ndes_immense inp, ndes_immense key, int *newkey, int isw, ndes_immense *out) {
        ...
        const volatile char ipm[65] = {0,  40, 8,  48, 16, 56, 24, 64, 32, 39, 7,  47, 15, 55, 23, 63, 31,
        ...
        if (initflag) {
            ...
            for (j = 2; j <= 32; j++) ndes_bit[j] = (shifter <<= 1);
        }
    ...
    void ndes_cyfun(unsigned long ir, ndes_great k, unsigned long *iout) {
        const volatile long iet[49] = {0,  32, 1,  2,  3,  4,  5,  4,  5,  6,  7,  8,  9,  8,  9,  10, 11,
                                       12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20, 21, 22,
                                       23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1};
        const volatile long ipp[33] = {0, 16, 7,  20, 21, 29, 12, 28, 17, 1,  15, 23, 26, 5,  18, 31, 10,
                                       2, 8,  24, 14, 32, 27, 3,  9,  19, 13, 30, 6,  22, 11, 4,  25};
        const volatile long is[16][4][9] = {{{0, 14, 15, 10, 7, 2, 12, 4, 13},
    ...
    void ndes_ks(/*ndes_immense key, */ int n, ndes_great *kn) {
        ...
        for (j = 16, k = 32, l = 48; j >= 1; j--, k--, l--) {
            (*kn).r = (*kn).r << 1;
            (*kn).r = ((*kn).r) | (unsigned short)ndes_getbit(ndes_icd, ndes_ipc2[j], 28);
            (*kn).c = (*kn).c << 1;
            (*kn).c = ((*kn).c) | (unsigned short)ndes_getbit(ndes_icd, ndes_ipc2[k], 28);
            (*kn).l = (*kn).l << 1;
            (*kn).l = ((*kn).l) | (unsigned short)ndes_getbit(ndes_icd, ndes_ipc2[l], 28);
        }
    }
