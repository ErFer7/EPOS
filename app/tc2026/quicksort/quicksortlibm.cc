...
float
    ...
    quicksort_zero = 0.0f, quicksort_huge = 1.0e30, quicksort_tiny = 1.0e-30f, quicksort_two = 2.0,
...
#endif
{
    ...
    /* y==zero: x**0 = 1 */
    if (iy == 0) return (quicksort_one);

    /* x==+-1 */
    if (x == 1.0f) return (quicksort_one);
    if ((x == -1.0f) && quicksort_isinf(y)) return (quicksort_one);
    ...
    */
    ...
    if ((ix == 0x7f800000) || (ix == 0) || (ix == 0x3f800000)) {
        z = ax;                            /*x is +-0,+-inf,+-1*/
        if (hy < 0) z = quicksort_one / z; /* z = (1/|x|) */
        if (hx < 0) {
    ...
    if (iy > 0x4d000000) { /* if |y| > 2**27 */
        ...
        if (ix < 0x3f7ffff8) return ((hy < 0) ? quicksort_huge * quicksort_huge : quicksort_tiny * quicksort_tiny);
        if (ix > 0x3f800007) return ((hy > 0) ? quicksort_huge * quicksort_huge : quicksort_tiny * quicksort_tiny);
        ...
        r = s2 * s2 *
            (quicksort_L1 +
             s2 * (quicksort_L2 + s2 * (quicksort_L3 + s2 * (quicksort_L4 + s2 * (quicksort_L5 + s2 * quicksort_L6)))));
        r += s_l * (s_h + s);
        s2 = s_h * s_h;
        t_h = (float)3.0f + s2 + r;
    ...
    if (((((unsigned int)hx >> 31) - 1) | (yisint - 1)) == 0) s = -quicksort_one; /* (-ve)**(odd int) */
    ...
    if (j > 0x43000000)                               /* if z > 128 */
        ...
        if (j == 0x43000000) {                                                           /* if z == 128 */
        if (p_l + quicksort_ovt > z - p_h) return (s * quicksort_huge * quicksort_huge); /* overflow */
    } else
        ...
        if (p_l <= z - p_h) return (s * quicksort_tiny * quicksort_tiny); /* underflow */
    }
    ...
    */
    ...
    w = v - (z - u);
    t = z * z;
    t1 = z - t * (quicksort_P1 + t * (quicksort_P2 + t * (quicksort_P3 + t * (quicksort_P4 + t * quicksort_P5))));
...
#endif
{
    ...
    if (ix <= 0x43490f80) { /* |x| ~<= 2^7*(pi/2), medium size */
        ...
        if ((n < 32) && ((int)(ix & 0xffffff00) != quicksort_npio2_hw[n - 1]))
...
#endif
{
    ...
    if (ix < 0x32000000) {                         /* if x < 2**27 */
        if (((int)x) == 0) return (quicksort_one); /* generate inexact */
    }

    z = x * x;
    r = z * (quicksort_C1 +
             z * (quicksort_C2 + z * (quicksort_C3 + z * (quicksort_C4 + z * (quicksort_C5 + z * quicksort_C6)))));
...
#endif
{
    ...
    z = x * x;
    v = z * x;
    r = quicksort_S2 + z * (quicksort_S3 + z * (quicksort_S4 + z * (quicksort_S5 + z * quicksort_S6)));

    if (iy == 0)
        return (x + v * (quicksort_S1 + z * r));
    else
        return (x - ((z * (quicksort_half * y - v * r) - y) - v * quicksort_S1));
}
...
#endif
{
    ...
    if (ix < 0x3ee00000) {                                      /* |x| < 0.4375 */
        if (ix < 0x31000000) {                                  /* |x| < 2^-29 */
            if (quicksort_huge + x > quicksort_one) return (x); /* raise inexact */
        }
        id = -1;
    } else {
        x = quicksort_fabsf(x);
    ...
    s1 = z * (quicksort_aT[0] +
              w * (quicksort_aT[2] +
                   w * (quicksort_aT[4] + w * (quicksort_aT[6] + w * (quicksort_aT[8] + w * quicksort_aT[10])))));
    s2 = w * (quicksort_aT[1] +
              w * (quicksort_aT[3] + w * (quicksort_aT[5] + w * (quicksort_aT[7] + w * quicksort_aT[9]))));

    if (id < 0)
        return (x - x * (s1 + s2));
    else {
        z = quicksort_atanhi[id] - ((x * (s1 + s2) - quicksort_atanlo[id]) - x);
        return ((hx < 0) ? -z : z);
    }
}
...
#endif
{
    ...
    /* |x| ~< pi/4 */
    ix &= 0x7fffffff;
    if (ix <= 0x3f490fd8) return (quicksort___kernel_cosf(x, z));
...
#endif
{
    ...
    /* |x| ~< pi/4 */
    ix &= 0x7fffffff;
    if (ix <= 0x3f490fd8) return (quicksort___kernel_sinf(x, z, 0));
