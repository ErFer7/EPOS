...
void Sha::sha_transform(struct SHA_INFO *sha_info) {
    int i;
    LONG temp, A, B, C, D, E, W[80];

    for (i = 0; i < 16; ++i) W[i] = sha_info->data[i];
...
void Sha::sha_byte_reverse(LONG *buffer, int count) {
    int i;
    BYTE ct[4], *cp;
...
Sha::Sha() {
    ...
    for (i = 0; i < 16; i++) sha_info.data[i] = 0;
}

size_t Sha::sha_fread(void *ptr, size_t size, size_t count, struct SHA_MY_FILE *stream) {
...
void Sha::sha_update(struct SHA_INFO *sha_info, BYTE *buffer, int count) {
    if ((sha_info->count_lo + ((LONG)count << 3)) < sha_info->count_lo) ++sha_info->count_hi;
...
void Sha::sha_final(struct SHA_INFO *sha_info) {
    int count;
...
void Sha::sha_stream(struct SHA_INFO *sha_info, struct SHA_MY_FILE *fin) {
    int i;
    BYTE data[BLOCK_SIZE];
    while ((i = sha_fread(data, 1, BLOCK_SIZE, fin)) > 0) sha_update(sha_info, data, i);
