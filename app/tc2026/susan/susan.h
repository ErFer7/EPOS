...
class Susan {
   ...
   public:
    Susan() {
        ...
        // drawing_mode=1; /* Mark corners/edges with single black points instead of black with white border; corners or
        // edges mode */ three_by_three=1; /* Use flat 3x3 mask, edges or smoothing mode */ susan_quick=1; /* Use faster
        // (and usually stabler) corner mode; edge-like corner suppression not carried out; corners mode */ dt=10.0; /*
   ...
   private:
    void susan_call_susan(struct wccFILE *inputFile, int mode) {
        ...
        if ((susan_principle_conf == 1) && (mode == 0)) mode = 1;
        ...
        switch (mode) {
            ...
            case 1:
                ...
                if (susan_principle_conf) {
                    ...
                    if (susan_thin_post_proc) susan_thin(r, mid, x_size, y_size);
    ...
    void
    susan_corners_quick(uchar *in, char *r, uchar *bp, int max_no, CORNER_LIST corner_list, int x_size, int y_size) {
        int n, x, y, i, j;
        uchar *p, *cp;
...
/* 5x5 mask */
                    ...
                    if ((x > r[(i - 1) * x_size + j + 2]) && (x > r[(i)*x_size + j + 1]) &&
...
#endif
#ifdef SEVEN_SUPP
                        if ((x > r[(i - 3) * x_size + j - 3]) && (x > r[(i - 3) * x_size + j - 2]) &&
                            ...
                            (x > r[(i)*x_size + j - 3]) && (x > r[(i)*x_size + j - 2]) && (x > r[(i)*x_size + j - 1]) &&
...
#endif
                        {
                            ...
                            corner_list[n].x = j;
                            corner_list[n].y = i;
                            x = in[(i - 2) * x_size + j - 2] + in[(i - 2) * x_size + j - 1] + in[(i - 2) * x_size + j] +
                                in[(i - 2) * x_size + j + 1] + in[(i - 2) * x_size + j + 2] +
                                in[(i - 1) * x_size + j - 2] + in[(i - 1) * x_size + j - 1] + in[(i - 1) * x_size + j] +
                                in[(i - 1) * x_size + j + 1] + in[(i - 1) * x_size + j + 2] + in[(i)*x_size + j - 2] +
                                in[(i)*x_size + j - 1] + in[(i)*x_size + j] + in[(i)*x_size + j + 1] +
                                in[(i)*x_size + j + 2] + in[(i + 1) * x_size + j - 2] + in[(i + 1) * x_size + j - 1] +
                                in[(i + 1) * x_size + j] + in[(i + 1) * x_size + j + 1] + in[(i + 1) * x_size + j + 2] +
                                in[(i + 2) * x_size + j - 2] + in[(i + 2) * x_size + j - 1] + in[(i + 2) * x_size + j] +
                                in[(i + 2) * x_size + j + 1] + in[(i + 2) * x_size + j + 2];
                            ...
                            x = in[(i - 2) * x_size + j + 2] + in[(i - 1) * x_size + j + 2] + in[(i)*x_size + j + 2] +
                                ...
                                (in[(i - 2) * x_size + j - 2] + in[(i - 1) * x_size + j - 2] + in[(i)*x_size + j - 2] +
                                 in[(i + 1) * x_size + j - 2] + in[(i + 2) * x_size + j - 2]);
                            x += x + in[(i - 2) * x_size + j + 1] + in[(i - 1) * x_size + j + 1] +
                                 in[(i)*x_size + j + 1] + in[(i + 1) * x_size + j + 1] + in[(i + 2) * x_size + j + 1] -
                                 (in[(i - 2) * x_size + j - 1] + in[(i - 1) * x_size + j - 1] + in[(i)*x_size + j - 1] +
                                  in[(i + 1) * x_size + j - 1] + in[(i + 2) * x_size + j - 1]);

                            y = in[(i + 2) * x_size + j - 2] + in[(i + 2) * x_size + j - 1] + in[(i + 2) * x_size + j] +
                                in[(i + 2) * x_size + j + 1] + in[(i + 2) * x_size + j + 2] -
                                (in[(i - 2) * x_size + j - 2] + in[(i - 2) * x_size + j - 1] +
                                 in[(i - 2) * x_size + j] + in[(i - 2) * x_size + j + 1] +
                                 in[(i - 2) * x_size + j + 2]);
                            y += y + in[(i + 1) * x_size + j - 2] + in[(i + 1) * x_size + j - 1] +
                                 in[(i + 1) * x_size + j] + in[(i + 1) * x_size + j + 1] +
                                 in[(i + 1) * x_size + j + 2] -
                                 (in[(i - 1) * x_size + j - 2] + in[(i - 1) * x_size + j - 1] +
                                  in[(i - 1) * x_size + j] + in[(i - 1) * x_size + j + 1] +
                                  in[(i - 1) * x_size + j + 2]);
    ...
    void susan_corners(uchar *in, char *r, uchar *bp, int max_no, CORNER_LIST corner_list, int x_size, int y_size) {
...
/* 5x5 mask */
    ...
    void susan_edges_small(uchar *in, char *r, uchar *mid, uchar *bp, int max_no, int x_size, int y_size) {
        float z;
    ...
    void susan_edges(uchar *in, char *r, uchar *mid, uchar *bp, int max_no, int x_size, int y_size) {
        float z;
        int do_symmetry, i, j, m, n, a, b, x, y, w;
        uchar c, *p, *cp;
            ...
            for (j = 4; j < x_size - 4; j++) {
                if (mid[i * x_size + j] < 8) {
                    ...
                    n = (*mp < 8) + (*(mp + 1) < 8) + (*(mp + 2) < 8) + (*(mp + x_size) < 8) +
                        (*(mp + x_size + 2) < 8) + (*(mp + x_size + x_size) < 8) + (*(mp + x_size + x_size + 1) < 8) +
...
#ifdef IGNORETHIS
                            if ((b00 & b01) | (b00 & b10) | (b02 & b01) | (b02 & b12) | (b20 & b10) | (b20 & b21) |
                                (b22 & b21) | (b22 & b12)) {
                                /* case; right angle ends. clean up.
                                   e.g.; X X O  CAN  become X X O
                                         O X O              O O O
                                         O O O              O O O        */
                                if (((b01) & (mid[(i - 2) * x_size + j - 1] > 7) & (mid[(i - 2) * x_size + j] > 7) &
                                     (mid[(i - 2) * x_size + j + 1] > 7) &
                                     ((b00 & ((2 * r[(i - 1) * x_size + j + 1]) > centre)) |
                                      (b02 & ((2 * r[(i - 1) * x_size + j - 1]) > centre)))) |
                                    ((b10) & (mid[(i - 1) * x_size + j - 2] > 7) & (mid[(i)*x_size + j - 2] > 7) &
                                     (mid[(i + 1) * x_size + j - 2] > 7) &
                                     ((b00 & ((2 * r[(i + 1) * x_size + j - 1]) > centre)) |
                                      (b20 & ((2 * r[(i - 1) * x_size + j - 1]) > centre)))) |
                                    ((b12) & (mid[(i - 1) * x_size + j + 2] > 7) & (mid[(i)*x_size + j + 2] > 7) &
                                     (mid[(i + 1) * x_size + j + 2] > 7) &
                                     ((b02 & ((2 * r[(i + 1) * x_size + j + 1]) > centre)) |
                                      (b22 & ((2 * r[(i - 1) * x_size + j + 1]) > centre)))) |
                                    ((b21) & (mid[(i + 2) * x_size + j - 1] > 7) & (mid[(i + 2) * x_size + j] > 7) &
                                     (mid[(i + 2) * x_size + j + 1] > 7) &
                                     ((b20 & ((2 * r[(i + 1) * x_size + j + 1]) > centre)) |
                                      (b22 & ((2 * r[(i + 1) * x_size + j - 1]) > centre))))) {
                                    mid[(i)*x_size + j] = 100;
    ...
    void susan_smoothing(int three_by_three, uchar *in, float dt, int x_size, int y_size, uchar *bp) {
        ...
        if (three_by_three == 0) {
            ...
            /* {{{ main section */
            // neue Änderung min max 107
            for (i = 1; i < y_size - 1; i++) {
                // neue Änderung min max 88
                for (j = 1; j < x_size - 1; j++) {
                    area = 0;
                    total = 0;
    ...
    void susan_get_image(struct wccFILE *fd, unsigned char **in, int *x_size, int *y_size) {
        char header[100];
    ...
    void susan_principle(uchar *in, char *r, uchar *bp, int max_no, int x_size, int y_size) {
        int i, j, n;
        uchar *p, *cp;
    ...
    void susan_principle_small(uchar *in, char *r, uchar *bp, int max_no, int x_size, int y_size) {
        int i, j, n;
        uchar *p, *cp;
    ...
    void susan_enlarge(uchar **in, uchar *tmp_image, int *x_size, int *y_size, int border) {
        ...
        for (i = 0; i < *y_size; i++) { /* copy *in into tmp_image */
            susan_wccmemcpy(tmp_image + (i + border) * (*x_size + 2 * border) + border, *in + i * *x_size, *x_size);
        }

        for (i = 0; i < border; i++) { /* copy top and bottom rows; invert as many as necessary */
            susan_wccmemcpy(tmp_image + (border - 1 - i) * (*x_size + 2 * border) + border, *in + i * *x_size, *x_size);
            susan_wccmemcpy(tmp_image + (*y_size + border + i) * (*x_size + 2 * border) + border,
                            *in + (*y_size - i - 1) * *x_size,
                            *x_size);
        }
