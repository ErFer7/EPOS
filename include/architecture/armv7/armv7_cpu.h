...
public:
    ...
    public:
        Context() {}
        Context(Log_Addr entry, Log_Addr exit, Log_Addr usp): ARMv7::Context(FLAG_THUMB, entry | thumb, exit | thumb) {}
...
public:
    ...
    public:
        Context() {}
        Context(Log_Addr entry, Log_Addr exit, Log_Addr usp): Multitask_Context(usp, exit), ARMv7::Context((multitask && usp) ? MODE_USR : MODE_SVC, entry, exit) {}
...
public:
    ...
    static Context * init_stack(Log_Addr usp, Log_Addr ksp, void (* exit)(), int (* entry)(Tn ...), Tn ... an) {
        ...
        if(usp) { // multitask
            ksp -= sizeof(Context);
            ctx = new(ksp) Context(&Context::first_dispatch, exit, 0);
        }
#endif
        return ctx;
    }
