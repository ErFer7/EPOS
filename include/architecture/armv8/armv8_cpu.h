...
if(!multitask)
    ASM("add sp, sp, #-16");                // skip ULR and USP
else {
    // TODO: handle multitask context push
}
...
if (!multitask)
    ...
    // TODO: handle multitask context pop
}
...
public:
    ...
    static Context * init_stack(Log_Addr usp, Log_Addr ksp, void (* exit)(), int (* entry)(Tn ...), Tn ... an) {
        ...
        if(multitask) {
            ksp -= sizeof(Context);
            ctx = new(ksp) Context(&context_load_helper, exit, 0);
