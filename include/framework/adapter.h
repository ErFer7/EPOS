...
public:
    ...
    unsigned long size() { enter(); unsigned long res = Component::size(); leave(); return res; }
    Segment::Phy_Addr phy_address() { enter(); CPU::Phy_Addr res = Component::phy_address(); leave(); return res; }
    long resize(long amount) { enter(); long res = Component::resize(amount); leave(); return res; }
