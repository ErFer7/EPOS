...
public:
    ...
    Log_Addr attach(Handle<Segment> * seg, Log_Addr addr) { return _stub->attach(*seg->_stub, addr); }
    void detach(Handle<Segment> * seg) { _stub->detach(*seg->_stub); }
    void detach(Handle<Segment> * seg, Log_Addr addr) { _stub->detach(*seg->_stub, addr); }
    Phy_Addr physical(const Log_Addr addr) { return _stub->physical(addr); }

    unsigned long size() const { return _stub->size(); }
    Segment::Phy_Addr phy_address() const { return _stub->phy_address(); }
    long resize(long amount) { return _stub->resize(amount); }
    ...
    // Timing
    static void delay(Microsecond t) { _Stub::delay(t); }
