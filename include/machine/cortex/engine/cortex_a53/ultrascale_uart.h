// EPOS ARM Cortex-A53 Xilinx Ultrascale+ ZU3EG - FZ3 - UART  Mediator Declarations

#ifndef __ultrascale_uart_h
#define __ultrascale_uart_h
...
class Xilinx_UART: public UART_Common
{
...
public:
    ...
    void config(unsigned int * baud_rate, unsigned int * data_bits, unsigned int * parity, unsigned int * stop_bits) {
        *data_bits = 0; //((uart(AUX_MU_LCR_REG) & 0x1) ? 8 : 7);
        *parity = 0; //by Manual definition
        *baud_rate = CLOCK; //CLOCK/(uart(AUX_MU_BAUD_REG)+1)/8;
        *stop_bits = 1; //by Manual definition
    }

    Reg8 rxd() { 
        return 0; //uart(AUX_MU_IO_REG); 
    }
    void txd(Reg8 c) { 
        ASM("                       \t\n\
            mov x1, #0xff000000     \t\n\
    ...
    bool rxd_ok() { 
        return true;//(uart(AUX_MU_LSR_REG) & RXF_LSR_DR); 
    }
    bool txd_ok() {
        Reg32 r;
        ASM("                       \t\n\
        ...
        // 
        return ((r >> 4) == 0);
        // return (uart(AUX_MU_LSR_REG) & TXF_LSR_EMPTY); 
    }

    bool rxd_full() { 
        return false;//(uart(AUX_MU_STAT_REG) & RX_STAT_LEVEL_FULL); 
    }
    bool txd_empty() { 
        return true;//(uart(AUX_MU_LSR_REG) & TXF_LSR_IDLE); 
    }

    bool busy() { 
        return false;
        // return !(uart(AUX_MU_LSR_REG) & TXF_LSR_IDLE); 
    }
