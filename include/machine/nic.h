...
public:
    ...
    public:
        ...
        constexpr Address(const Null &) {
            for(unsigned int i = 0; i < LENGTH; i++)
                _address[i] =  NULL;
        }

        constexpr Address(const Broadcast &) {
            for(unsigned int i = 0; i < LENGTH; i++)
        ...
        Address(unsigned int a) {
            ...
            a = htonl(a);
            memcpy(this, &a, sizeof(unsigned int));
        }
    ...
    struct Dummy_Metadata
    {
        ...
        friend OStream & operator<<(OStream & os, const Dummy_Metadata & m) {
            os << "{no metadata}";
            return os;
        }
    };
    ...
    struct TSTP_Metadata
    {
        ...
        friend OStream & operator<<(OStream & os, const TSTP_Metadata & m) {
            os << "{rssi=" << m.rssi << ",period=" << m.period << ",sfdts=" << m.sfdts << ",id=" << m.id
