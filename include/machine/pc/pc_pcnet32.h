...
public:
    ...
    struct Desc {
        enum {
            OWN = 0x8000,       // Who owns the buffer? 1 => NIC, 0 => Host
            ERR = 0x4000,
            STP = 0x0200,
            ENP = 0x0100,
            BPE = 0x0080
...
protected:
    PCNet32(unsigned int unit, IO_Port io_port, IO_Irq irq, DMA_Buffer * dma_buf);
...
private:
    ...
    static PCNet32 * get_by_interrupt(Interrupt_Id interrupt) {
