// EPOS VisionFive2 (RISC-V) Metainfo and Configuration
...
public:
    ...
    static const bool supervisor = true; // Run EPOS library in machine mode (works in supervisor as well)
    ...
    // Clocks
    static const unsigned long OSC              = 24000000; // Base oscillator frequency (24MHz)
    ...
    static const unsigned long RTCCLK           = 4000000; // The CPU real time clock (rtcclk) runs at 1 MHz and is driven from input pin RTCCLKIN. This should be connected to an external oscillator.
    ...
    static const unsigned long RAM_TOP          = 0x47ffffff;  // 2 GB + 128 MB (max 256 GB of RAM + MIO)
    ...
    static const unsigned int HEAP_SIZE         = 4 * 1024 * 1024;  // 4 MB
};
...
template <> struct Traits<IC> : public Traits<Machine_Common>
{
    ...
    static const unsigned int PLIC_IRQS = 136; // IRQ0 is used by PLIC to signalize that there is no interrupt being serviced or pending
...
{
    typedef LIST<DWC_Ether_QoS> DEVICES;
...
template <> struct Traits<DWC_Ether_QoS> : public Traits<Ethernet>
{
    static const unsigned int UNITS = DEVICES::Count<DWC_Ether_QoS>::Result;
    static constexpr unsigned int IRQs[] = {39};
