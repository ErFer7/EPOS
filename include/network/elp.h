...
public:
    ...
    public:
        ...
        friend OStream & operator<<(OStream & os, const Packet & p) {
            os << "{head=" << reinterpret_cast<const Header &>(p) << ",data=" << p._data << "}";
            return os;
        }
...
public:
    ...
    static int send(const Address & from, const Address & to, const void * data, unsigned int size) {
        ...
        if(!binding) {
            db<ELP>(WRN) << "ELP::send: cannot bind to port " << from.port() << "!" << endl;
            return 0;
        }
        ELP * elp = binding->a();
        ...
        if(!buf) {
            db<ELP>(WRN) << "ELP::send: cannot allocate buffer of " << size << " bytes!" << endl;
            return 0;
        }
