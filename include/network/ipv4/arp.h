...
private:
    static const unsigned int ENTRIES = Traits<Build>::NETWORKING;
...
private:
    ...
    public:
        ...
        friend OStream & operator<<(OStream & os, const Mapping & m) {
            os << "{pa=" << m._link.key() << ",ha=" << m._ha << ",sem=" << m._sem << "}";
            return os;
        }
