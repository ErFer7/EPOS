...
void CPU_Affinity::handle(Event event)
{
    unsigned int laziest_cpu = ANY;
//    unsigned int busier_cpu = ANY;
    Tick laziest_cpu_time = -1U;
//    Tick busier_cpu_time = 0;

    Scheduling_Criterion_Common::handle(event);

    if(CREATION) {
        for(unsigned int i = 0; i < Traits<Build>::CPUS; i++) {
            if(laziest_cpu_time > _statistics.cpu_time[i]) {
                laziest_cpu_time = _statistics.cpu_time[i];
                laziest_cpu = i;
            }
//            if(busier_cpu_time < _statistics.cpu_time[i]) {
//                busier_cpu_time = _statistics.cpu_time[i];
//                busier_cpu = i;
//            }
        }

        _next_queue = laziest_cpu;
    }
}
