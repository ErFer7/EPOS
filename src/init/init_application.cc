...
public:
    Init_Application() {
        ...
        // Only the bootstrap CPU runs INIT_APPLICATION on non-kernel configurations (on kernel configurations, Init_Application is linked to the APP, not the SYS).
        ...
        if(Traits<System>::multiheap) { // heap in data segment arranged by SETUP
            ...
            Application::_heap = new (&Application::_preheap[0]) Application::Heap(heap, HEAP_SIZE);
        } else {
