...
#endif
#ifdef __cortex_m__
extern "C" { void _dispatch(EPOS::S::IC_Common::Interrupt_Id) __attribute__ ((alias("_ZN4EPOS1S2IC8dispatchEm"))); }
#endif
...
void IC::prefetch_abort()
{
    ...
    if((CPU::r1()) == CPU::Reg(&__exit)) {
        db<IC,Machine>(TRC) << "IC::prefetch_abort: thread's final return!" << endl;
       __exit();
    } else
