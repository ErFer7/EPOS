// EPOS Risc-V GEM (Cadence) Ethernet NIC Mediator Implementation
...
int GEM::receive(Address * src, Protocol * prot, void * data, unsigned int size)
{
    ...
    *prot = frame->prot();
    ...
    if(Buffer::Metadata::collect_sfdts)
        buf->sfdts = (Traits<GEM>::EXPECTED_SIMULATION_TIME > 0) ? TSC::time_stamp() : desc->time_stamp(); // QEMU doesn't seem to set the timestamp
...
GEM::Buffer * GEM::alloc(const Address& dst, const Protocol& prot, unsigned int once, unsigned int always, unsigned int payload)
{
    ...
    for(int size = once + payload; size > 0; size -= max_data) {
        ...
        buf->fill(actual_size, _configuration.address, dst, prot);

        desc->size(actual_size + sizeof(Header));
        desc->ctrl &= ~Tx_Desc::OWN; // Give the NIC ownership of the buffer (so it can send it)
...
int GEM::send(Buffer * buf)
{
    ...
    for(Buffer::Element * el = buf->link(); el; el = el->next()) {
        ...
        db<GEM>(TRC) << "GEM::send(buf=" << buf << ")" << endl;
...
void GEM::free(Buffer * buf)
{
    ...
    for(Buffer::Element* el = buf->link(); el; el = el->next()) {
        ...
        // Release the buffer to the NIC
        desc->size(sizeof(Frame) - sizeof(Trailer));
...
void GEM::int_handler(IC::Interrupt_Id interrupt)
{
    GEM * dev =  get_by_interrupt(interrupt);
