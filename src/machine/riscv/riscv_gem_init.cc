// EPOS Risc-V GEM (Cadence) Ethernet NIC Mediator Initialization
...
GEM::GEM(unsigned int unit, DMA_Buffer* dma_buf)
{
    ...
    for(unsigned int i = 0; i < RX_BUFS; i++) {
        ...
        _rx_ring[i].ctrl = 0;
        _rx_ring[i].size(sizeof(Frame) - sizeof(Trailer));
    ...
    for(unsigned int i = 0; i < TX_BUFS; i++) {
        ...
        _tx_ring[i].addr = phy;
        _tx_ring[i].ctrl = Tx_Desc::OWN | Tx_Desc::LAST_BUF; // Owned by host
