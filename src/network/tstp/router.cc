...
void TSTP::Router::update(Data_Observed<Buffer> * obs, Buffer * buf)
{
    ...
    if(buf->is_microframe) {
        ...
        if((header->type() == CONTROL) && (header->subtype() == KEEP_ALIVE)) {
            ...
            buf->destined_to_me = (((header->origin().space == here()) && ((dest.center() == Region::Center(0,0,0)) && (dest.radius() == 0)) && (dest.contains(here(), dest.t0))) || ((header->origin().space != here()) && (dest.contains(here(), dest.t0))));
...
void TSTP::Router::marshal(Buffer * buf)
{
    ...
    buf->destined_to_me = (((buf->frame()->data<Header>()->origin().space == here()) && ((dest.center() == Region::Center(0,0,0)) && (dest.radius() == 0)) && (dest.contains(here(), dest.t0))) || ((buf->frame()->data<Header>()->origin().space != here()) && (dest.contains(here(), now()))));
