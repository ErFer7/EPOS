...
OStream & operator<<(OStream & os, const TSTP::Packet & p)
{
    switch(p.type()) {
    case TSTP::INTEREST:
        os << reinterpret_cast<const TSTP::Interest &>(p);
        break;
    case TSTP::RESPONSE:
        os << reinterpret_cast<const TSTP::Response &>(p);
        break;
    case TSTP::COMMAND:
        os << reinterpret_cast<const TSTP::Command &>(p);
        break;
    case TSTP::CONTROL:
        switch(p.subtype()) {
        case TSTP::DH_RESPONSE:
            os << reinterpret_cast<const TSTP::Security::DH_Response &>(p);
            break;
        case TSTP::AUTH_REQUEST:
            os << reinterpret_cast<const TSTP::Security::Auth_Request &>(p);
            break;
        case TSTP::DH_REQUEST:
            os << reinterpret_cast<const TSTP::Security::DH_Request &>(p);
            break;
        case TSTP::AUTH_GRANTED:
            os << reinterpret_cast<const TSTP::Security::Auth_Granted &>(p);
            break;
        case TSTP::REPORT:
            os << reinterpret_cast<const TSTP::Security::Report &>(p);
            break;
        case TSTP::KEEP_ALIVE:
            os << reinterpret_cast<const TSTP::Timekeeper::Keep_Alive &>(p);
            break;
        case TSTP::EPOCH:
            os << reinterpret_cast<const TSTP::Timekeeper::Epoch &>(p);
            break;
//        case TSTP::MODEL:
