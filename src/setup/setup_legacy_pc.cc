...
void Setup::setup_idt()
{
    ...
    // Clear IDT
    memset(reinterpret_cast<void *>(idt), 0, sizeof(Page));
...
void Setup::setup_gdt()
{
    ...
    // Clear GDT
    memset(reinterpret_cast<void *>(gdt), 0, sizeof(Page));
...
void Setup::setup_sys_pt()
{
    ...
    memset(reinterpret_cast<void *>(sys_pt), 0, pts * sizeof(Page_Table));
    ...
    // BUG: the line bellow makes QEMU crash when EPOS is a library; note that the same code is used in other parts of this program without any issues!
//    db<Setup>(INF) << "SYS_PT[" << sys_pt << "]=" << *reinterpret_cast<Page_Table *>(sys_pt) << endl;
}
...
void Setup::setup_tss()
{
    ...
    tss->esp0 = SYS_STACK + Traits<System>::STACK_SIZE * (CPU::id() + 1) - 2 * sizeof(Reg); // APs' tss->esp0 will be reconfigured later by CPU::Context::load()
