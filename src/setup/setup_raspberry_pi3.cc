...
void Setup::build_pmm()
{
    ...
    top_page -= MMU::pts(MMU::pages(SYS_HEAP - SYS));
...
void Setup::setup_sys_pt()
{
    ...
    unsigned int n_pts = MMU::pts(MMU::pages(SYS_HIGH - SYS));

    // Clear the System Page Table
    memset(reinterpret_cast<void *>(sys_pt), 0, n_pts * sizeof(Page_Table));

    // System Info
    sys_pt[MMU::pti(SYS, SYS_INFO)] = MMU::phy2pte(si->pmm.sys_info, Flags::SYSD);

    // Set an entry to this page table, so the system can access it later
    sys_pt[MMU::pti(SYS, SYS_PT)] = MMU::phy2pte(si->pmm.sys_pt, Flags::SYSD);

    // System Page Directory
    sys_pt[MMU::pti(SYS, SYS_PD)] = MMU::phy2pte(si->pmm.sys_pd, Flags::SYSD);
    ...
    for(i = 0, aux = si->pmm.sys_code; i < MMU::pages(si->lm.sys_code_size); i++, aux = aux + sizeof(Page))
        sys_pt[MMU::pti(SYS, SYS_CODE) + i] = MMU::phy2pte(aux, Flags::SYSC);

    // SYSTEM data
    for(i = 0, aux = si->pmm.sys_data; i < MMU::pages(si->lm.sys_data_size); i++, aux = aux + sizeof(Page))
        sys_pt[MMU::pti(SYS, SYS_DATA) + i] = MMU::phy2pte(aux, Flags::SYSD);
    ...
    for(i = 0, aux = si->pmm.sys_stack; i < MMU::pages(si->lm.sys_stack_size); i++, aux = aux + sizeof(Page))
        sys_pt[MMU::pti(SYS, SYS_STACK) + i] = MMU::phy2pte(aux, Flags::SYSD);
...
void Setup::setup_app_pt()
{
    ...
    memset(reinterpret_cast<void *>(app_code_pt), 0, n_pts_code * sizeof(Page_Table));
    memset(reinterpret_cast<void *>(app_data_pt), 0, n_pts_data * sizeof(Page_Table));
    ...
    for(i = 0, aux = si->pmm.app_code; i < MMU::pages(si->lm.app_code_size); i++, aux = aux + sizeof(Page))
        app_code_pt[MMU::pti(si->lm.app_code) + i] = MMU::phy2pte(aux, Flags::APPC);
    ...
    for(i = 0, aux = si->pmm.app_data; i < MMU::pages(si->lm.app_data_size); i++, aux = aux + sizeof(Page))
        app_data_pt[MMU::pti(si->lm.app_data) + i] = MMU::phy2pte(aux, Flags::APPD);
...
void Setup::setup_sys_pd()
{
    ...
    memset(reinterpret_cast<void *>(sys_pd), 0, sizeof(Page_Directory));
    ...
    for(unsigned int i = MMU::pti(si->bm.mem_base), j = 0; i < MMU::pti(si->bm.mem_base) + mem_size; i++, j++)
        pts[i] = MMU::phy2pte(si->bm.mem_base + j * sizeof(Page), Flags::SYSD);

    // Attach all the physical memory starting at PHY_MEM
    assert((MMU::pdi(MMU::align_segment(PHY_MEM)) + n_pts) < (MMU::PD_ENTRIES - 1)); // check if it would overwrite the OS
    for(unsigned int i = MMU::pdi(MMU::align_segment(PHY_MEM)), j = 0; i < MMU::pdi(MMU::align_segment(PHY_MEM)) + n_pts; i++, j++)
        sys_pd[i] = MMU::phy2pde(si->pmm.phy_mem_pt + j * sizeof(Page_Table));

    // Attach the portion of the physical memory used by Setup at SETUP
    assert(MMU::pdi(SETUP) == MMU::pdi(RAM_BASE));
    sys_pd[MMU::pdi(SETUP)] =  MMU::phy2pde(si->pmm.phy_mem_pt);

    // Attach all the physical memory starting at RAM_BASE (used in library mode)
    assert((MMU::pdi(MMU::align_segment(RAM_BASE)) + n_pts) < (MMU::PD_ENTRIES - 1)); // check if it would overwrite the OS
    if(RAM_BASE != PHY_MEM)
    ...
    for(unsigned int i = MMU::pdi(MMU::align_segment(SYS)), j = 0; i < MMU::pdi(MMU::align_segment(SYS)) + n_pts; i++, j++)
        sys_pd[i] = MMU::phy2pde(si->pmm.sys_pt + j * sizeof(Page_Table));

    // Attach the first APPLICATION CODE (i.e. app_code_pt)
    n_pts = MMU::pts(MMU::pages(si->lm.app_code_size));
    for(unsigned int i = MMU::pdi(MMU::align_segment(si->lm.app_code)), j = 0; i < MMU::pdi(MMU::align_segment(si->lm.app_code)) + n_pts; i++, j++)
        sys_pd[i] = MMU::phy2pde(si->pmm.app_code_pt + j * sizeof(Page_Table));

    // Attach the first APPLICATION DATA (i.e. app_data_pt, containing heap, stack and extra)
    n_pts = MMU::pts(MMU::pages(si->lm.app_data_size));
    for(unsigned int i = MMU::pdi(MMU::align_segment(si->lm.app_data)), j = 0; i < MMU::pdi(MMU::align_segment(si->lm.app_data)) + n_pts; i++, j++)
        sys_pd[i] = MMU::phy2pde(si->pmm.app_data_pt + j * sizeof(Page_Table));
...
void Setup::call_next()
{
    ...
    if(multitask) {
        ...
        // Next stage will use SYS_STACK instead of BOOT_STASCK (the 2 integers on the stacks are room for the return address)
        Log_Addr sp = SYS_STACK + Traits<System>::STACK_SIZE * (CPU::id() + 1) - 2 * sizeof(CPU::Reg);
