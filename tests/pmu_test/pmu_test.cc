...
int main()
{
    ...
    PMU::config(0, PMU_Event::INSTRUCTIONS_RETIRED);
    PMU::config(1, PMU_Event::UNHALTED_CYCLES);
    PMU::config(2, PMU_Event::CPU_CYCLES);
    PMU::config(3, PMU_Event::LAST_LEVEL_CACHE_HITS);
    PMU::config(4, PMU_Event::BRANCHES);
    return 1;
...
//    unsigned long long llc_hit = 0;
    ...
    //perf.llc_misses();
    //perf.llc_hit();
    PMU::config(0, PMU_Event::LAST_LEVEL_CACHE_MISSES);
    c.reset();
